...
int bfs() {
	...
	while (!q.empty()) {
		...
		//cout << "bfs path=";
		for (i = 0; i <= n; i++) {//遍历节点个数
			int y = Edge[t][i].c - Edge[t][i].f;//增广路径余量
			if (i != start && path[i] == -1 && y) {
				_flow[i] = _flow[t] < y ? _flow[t] : y;//记录最小值
				q.push(i);//进队
				path[i] = t;//记录路径
				//cout << t << ", ";
			}
...
void Edmonds_Karp() {
	...
	while (true) {
		...
		while (now != start) {//更新边容量
			pre = path[now];
			Edge[pre][now].f += step;      //更新正向边的实际容量  
			Edge[now][pre].f -= step;      //添加反向边  
			now = pre;
			_tran++;
		}
